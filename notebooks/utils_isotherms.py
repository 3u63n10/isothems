...
def dubinin_ra(Ce, Qmax, Ea, nDA, Cref=1.0, T=298.0):
    """Dubinin-Astakhov: q = Qmax*exp[-(eps/Ea)^nDA], eps = RT*ln(1 + Cref/Ce)."""
    RT = 8.314e-3 * T  # kJ/mol
    eps = RT * np.log(1 + Cref / Ce)
...
def extract_descriptors(model_name, params, C0=1.0, T=298.0, Cref=1.0):
    """
    ...
    Cref : float   reference concentration for D-R/D-A, in the same units as Ce
    ...
    if model_name == 'Langmuir':
        ...
        Ea = params['Ea']
        nDA = params['nDA']
        # C_1/2 from Ea: eps = Ea*(ln2)^(1/nDA) = RT*ln(1+Cref/C_1/2)
        eps_half = Ea * np.log(2)**(1.0 / nDA)
        C_half = Cref / (np.exp(eps_half / RT) - 1)
        Kaff = C0 / C_half
        sigma_E = Ea * np.sqrt(
