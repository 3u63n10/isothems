...
Provides:
  ...
  - Universal descriptor extraction (Qmax, K*aff, Eads, sigma_E)
...
def extract_descriptors(model_name, params, C0=1.0, T=298.0):
    """
    Extract {Qmax, K*aff, Eads, sigma_E} from fitted parameters.
    ...
    if model_name == 'Langmuir':
        ...
        Qmax = params['Q1'] + params['Q2']
        # Report per-site values; use average for fingerprint
        K1, K2 = params['K1'], params['K2']
        Kaff = np.sqrt(K1 * K2) * C0  # geometric mean
        sigma_E = RT * np.abs(np.log(K1 / K2))  # site separation
        ...
        t = params['t']
        sigma_E = np.pi * RT / np.sqrt(3) / t  # rough approx
        ...
        bT = params['bT']
        Kaff = aT * C0
        Qmax = np.nan  # Temkin has no explicit Qmax
        sigma_E = bT * Qmax / (2 * np.sqrt(3)) if not np.isnan(Qmax) else np.nan
...
def fE_sips(E, Ec, n, Qmax, T=298.0):
    """Sips: logistic density."""
    RT = 8.314e-3 * T
    u = (E - Ec) / (n * RT)
    eu = np.exp(u)
    return Qmax / (n * RT) * eu / (1 + eu)**2
