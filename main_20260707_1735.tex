...
% New no-overwrite manuscript copy created from main_20260707_1641.tex after the user requested corrections from the attached evaluation.
% Codex updated the SVM equation reference, filled the ACS suppinfo summary, softened intrinsic-heterogeneity wording,
% aligned the Freundlich statement with reference-dependent apparent affinity, strengthened the minimum-data rule,
% replaced universal-descriptor wording with standardized reporting descriptors, and cleaned review-visible comments.
...
% --- External Documents ---
\externaldocument[SI-]{SI_20260707_1735}
...
% Draft note: prose retained in the surrounding paragraph.
...
% =================================================================
	...
	Applying eq~\eqref{eq:fE_general} to each isotherm yields the energy-distribution families summarized in Table~\ref{tab:energy_distributions}. These distributions should be interpreted as the model-implied site-energy distributions under the condensation approximation.
...
Importantly, the use of information criteria does not imply that one model is the ``true'' model, but rather identifies the model that achieves the best trade-off between accuracy and parsimony within the candidate set. In the context of adsorption isotherms, this is essential to avoid overinterpreting parameters obtained from overly flexible models that fit noise rather than underlying adsorption behavior.
...
For classification and clustering, the reporting fingerprint is converted into the standardized feature vector 
\begin{equation}
	\mathbf{x}
	=
	\left(
	...
	\sigma_H
	\right).
	\label{eq:classification_features}
\end{equation}
Here, \(Q_{\max}\) captures capacity, \(\ln K_{\mathrm{aff}}^{\ast}\) captures standard-state affinity on an additive scale, and \(\sigma_H\) captures the reported model-implied heterogeneity.

Once the feature vector in Eq.~\eqref{eq:classification_features} has been extracted for a collection of adsorbent--adsorbate systems, materials can be classified in this descriptor space without imposing a parametric model on the group boundaries.
The $k$-Nearest Neighbours (KNN) algorithm assigns a query material to the class most common among its $k$ closest neighbours:
	...
	\begin{equation}
		...
		\label{eq:SVM}
	\end{equation}
	where $\mathbf{w}$ is the normal to the hyperplane, $b$ is the bias, $\xi_i$ are slack variables for misclassified points, and $C_{\mathrm{reg}}$ controls the trade-off between margin width and classification error. The support vectors---the data points that lie on or within the margin---define the boundary entirely. Figure~\ref{fig:SVM}a shows a linear SVM separating two classes of adsorbents in the $(Q_{\max},\, \ln K_{\mathrm{aff}}^{\ast})$ plane, corresponding to a two-dimensional projection of the feature vector in Eq.~\eqref{eq:classification_features}.
	...
	Both KNN and SVM operate on the same standardized descriptor table (Eq.~\ref{SI-eq:si_standardise}) with Euclidean distance (Eq.~\ref{SI-eq:si_distance}). Ward's hierarchical clustering (Eq.~\ref{SI-eq:si_Ward}) provides an unsupervised alternative when class labels are unknown. The key advantage is that all three methods operate on model-harmonized descriptors, ensuring that the grouping reflects standardized model-derived adsorption behavior rather than artefacts of different fitting models. The full mathematical details of all clustering and classification methods are given in Section~\ref{SI-sec:si_clustering}.
% =================================================================
...
% =================================================================
	...
	\begin{enumerate}
		\item \textbf{Data collection.} Record $q_e$--$C_e$ pairs at constant $T$ and pH, with at least \(n \geq 8\) points as an absolute minimum, and preferably \(n \geq p+5\) for every candidate model considered, spanning the linear, transition, and plateau regions whenever saturation descriptors are to be reported.
	...
	\begin{suppinfo}
The Supporting Information contains the full isotherm equations and limiting cases, residual error metrics, information criteria, model-selection workflow, energy-distribution derivations, descriptor-extraction tables, reporting protocol, and documentation of the companion notebooks.
\end{suppinfo}
