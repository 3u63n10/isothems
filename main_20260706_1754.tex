...
% This timestamped main-text copy preserves the reviewed manuscript state after
% aligning the descriptor language with intrinsic heterogeneity width sigma_H,
% updating the Sips/Dubinin discussion points, and linking the main narrative
% to the notebook-based fitting, validation, and classification workflow.
...
\begin{document}
	...
	Fourth, a simple machine learning explanation using k-nearest neighbors (KNN) and support-vector machines (SVM) to classify materials according to their performance.
...
\begin{comment}
	...
	\begin{itemize}
		...
		\item \textbf{Dubinin models.} D-R and D-A operate in the Polanyi adsorption potential $\varepsilon = RT\ln(1+C_{\mathrm{ref}}/C_e)$, where \(C_{\mathrm{ref}}\) is reported in the same concentration units as \(C_e\). The distribution in $\varepsilon$ is Weibull; the D-R case ($n_{DA} = 2$) gives the Rayleigh distribution with $\sigma_\varepsilon \approx 0.463\,E_a$.
	\end{itemize}
	...
	The apparent standard deviation $\sigma_E$ describes the mathematical site-energy distribution. For reporting, we use the intrinsic heterogeneity width $\sigma_H$, obtained by removing the Langmuir thermal-kernel width from continuous heterogeneous models such as Sips, Koble--Corrigan, and Hill. Thus the homogeneous Langmuir limit is continuous: as the heterogeneity exponent approaches one, $\sigma_H \to 0$. Small $\sigma_H$ indicates a nearly homogeneous surface, whereas large $\sigma_H$ signals broad intrinsic energetic diversity typical of activated carbons, biochars, or disordered porous solids. Numerical apparent-width values and the conversion to the reported descriptor are tabulated in Table~\ref{SI-tab:si_sigma_numerical}.
	
	The usefulness of $\sigma_H$ extends beyond describing surface heterogeneity. Because it is derived from the fitted isotherm parameters on the same footing as $Q_{\max}$ and $E_{\mathrm{ads}}$, it converts model-specific heterogeneity into a common statistical descriptor. This enables comparison of competing models not only through residual-based criteria such as SSE, AIC$_c$, BIC, or F-tests, but also through the physical similarity of their implied site-energy distributions, for example by overlap metrics. More broadly, the descriptor set $\{Q_{\max}, E_{\mathrm{ads}}, \sigma_H\}$ defines a common feature space in which adsorbent--adsorbate systems can be grouped, ranked, or separated using statistical learning tools such as k-Nearest Neighbours (KNN) and Support Vector Machines (SVM). In this way, the site-energy distribution becomes the link between isotherm physics, statistical model comparison, and data-driven classification.
...
\end{comment}
	...
	Once an isotherm has been mapped onto an energy distribution, two quantities become especially useful: the central energy scale and the distribution width. In the present framework, the central position is represented by the adsorption energy $E_{\mathrm{ads}}$ introduced in Section~\ref{sec:Eads}, while the reported energetic dispersion is represented by the intrinsic width $\sigma_H$. Together they summarize where the distribution is centered and how broad the intrinsic heterogeneity is.
	
	The width $\sigma_H$ provides a direct measure of intrinsic heterogeneity and is obtained analytically from the same fitted parameters used to determine $E_{\mathrm{ads}}$, imposing no additional fitting burden. Small values, $\sigma_H \ll RT$, indicate near-homogeneous surfaces. Values on the order of $RT$ indicate moderate heterogeneity. Values much larger than $RT$ indicate broad energetic diversity, as commonly found in activated carbons, biochars, and disordered porous materials. The SI distinguishes the apparent mathematical width $\sigma_E$ from the reported intrinsic width $\sigma_H$ so that Langmuir is recovered at $\sigma_H=0$. Representative numerical values are given in Table~\ref{SI-tab:si_sigma_numerical}.
...
\begin{comment}
	...
	\begin{itemize}
		...
		\item $\sigma_H$ --- intrinsic heterogeneity width (kJ\,mol$^{-1}$), quantifying the degree of surface heterogeneity after the Langmuir thermal baseline is removed where applicable.
	\end{itemize}
...
Importantly, the use of information criteria does not imply that one model is “true,” but rather identifies the model that achieves the best trade-off between accuracy and parsimony within the candidate set. In the context of adsorption isotherms, this is essential to avoid overinterpreting parameters obtained from overly flexible models that fit noise rather than underlying adsorption behavior.
	...
	\begin{itemize}
		\item \textbf{Langmuir vs.\ Sips:} Accept Langmuir if the Sips-derived intrinsic heterogeneity is negligible, $\sigma_H^{\mathrm{(Sips)}} = (\pi RT/\sqrt{3})\sqrt{1/m^2-1} \leq \alpha\,RT$ with $\alpha \approx 1$ as a practical screen; Eq.~\ref{SI-eq:si_criterion_LvsS}.
		...
		\item \textbf{Sips vs.\ Double Langmuir:} Prefer Sips when the site-energy separation is smaller than the intrinsic Sips width, $\Delta E_{12} < 2\,\sigma_H^{\mathrm{(Sips)}}$ (Eq.~\ref{SI-eq:si_criterion_SvsDL}).
	\end{itemize}
	...
	\begin{itemize}
		...
		\item \textbf{Homogeneity screening:} separating materials with near-homogeneous surfaces ($\sigma_H \approx 0$) from those with broad intrinsic heterogeneity ($\sigma_H \gg RT$).
	\end{itemize}
	...
	\end{enumerate}
	The companion notebooks implement this same workflow, including fit overlays,
	residual and parity plots, metric summaries, covariance-based confidence
	bands, bootstrap and noise-sensitivity propagation, and KNN/SVM/Ward
	validation of descriptor tables. These code-level outputs are mapped to the
	SI equations and reporting rules in Section~\ref{SI-sec:si_notebooks}.
	...
	\begin{itemize}
		...
		\item \textbf{Use $\sigma_H$ to assess model necessity.} If the best 3-parameter model yields only weak intrinsic heterogeneity, e.g.\ $\sigma_H \leq 2RT$ or, for Sips, $m$ remains close to 1, the Langmuir model is likely sufficient and the extra parameter is not justified (Eq.~\ref{SI-eq:si_criterion_LvsS}).
