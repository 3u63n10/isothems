...
\begin{document}
	...
	In this work, we propose a practical set of reporting guidelines grounded in the following principles. First, capacity comparisons should be restricted to isotherm models for which a finite saturation is explicitly defined---that is, models with a well-defined $Q_{\text{max}}$, such as Langmuir, double Langmuir, or other saturating forms---while the Freundlich equation should be recognized as unsuitable for reporting a maximum capacity. A full explanation of the available isotherms is included in the SI.
	...
	Third, a statistical guideline to evaluate errors beyond the simple and well-known $R^2$. This section also incorporates a decision chart to guide the user of this method in choosing the best model to fit their experimental data.
	Fourth, a simple Machine learning explanation using $k$-Nearest Neighbors (KNN) and Support-Vector Machines (SVM) to cluster materials according to their performance.
	All this information is introduced in three stages: introduction and explanation in the main body, mathematical fundamentals in the SI, and a series of companion Jupyter notebooks with synthetic datasets to familiarize the user with the method and fit their own data.
...
\begin{equation}
	...
	\label{eq:freundlich}
\end{equation}
$K_F$ has concentration-dependent units (mg$\cdot$g$^{-1}$)(L$\cdot$mg$^{-1}$)$^{1/n}$; no $Q_{\max}$ is defined.
...
In this construction, $Q_{\max}$ removes the dependence of the Henry slope on the total adsorption capacity, while $C^0$ removes the remaining concentration unit. Thus, $K_{\mathrm{aff}}^{\ast}$ represents the intrinsic affinity of the adsorbent--adsorbate pair relative to a defined concentration standard. For the Langmuir isotherm, for example, $K_H = Q_{\max} b$, and therefore $K_{\mathrm{aff}}^{\ast}=bC^{0}$, recovering the usual dimensionless form of the Langmuir equilibrium constant. The same procedure can be applied to any isotherm model for which a finite Henry limit and a meaningful saturation capacity can be defined.
...
\begin{equation}
	...
	\label{eq:Eads}
\end{equation}
Because $K_{\mathrm{aff}}^{\ast}$ is dimensionless, the logarithmic term is also dimensionless:
\begin{equation}
	[\ln K_{\mathrm{aff}}^{\ast}] = 1.
\end{equation}
Therefore, the units of $E_{\mathrm{ads}}$ arise exclusively from the product $RT$. If the gas constant is expressed as
\begin{equation}
...
\begin{equation}
	...
	[RT]
	=
	\left(\mathrm{kJ\,mol^{-1}\,K^{-1}}\right)
	\left(\mathrm{K}\right)
	=
	\mathrm{kJ\,mol^{-1}}.
\end{equation}

Thus, $E_{\mathrm{ads}}$ has units of $\mathrm{kJ\,mol^{-1}}$ and is thermodynamically well defined only because the equilibrium constant inside the logarithm is dimensionless. Since $K_{\mathrm{aff}}^{\ast}$ is constructed from the Henry-region affinity and normalized by a common standard-state concentration, $E_{\mathrm{ads}}$ provides a model-independent measure of adsorption affinity that can be compared across adsorbents, contaminants, and laboratories.

It is critical to note, however, that when comparing different adsorbates (cross-pollutant studies, e.g., comparing PFAS with heavy metals or pharmaceuticals), the choice of units for the standard-state concentration $C^0$ becomes thermodynamically sensitive. If a mass-based standard state is selected (such as $C^0 = 1$~mg~L$^{-1}$), the resulting $E_{\mathrm{ads}}$ will be implicitly biased by the molecular weight of each solute. For example, two contaminants with identical molar adsorption energies will display different mass-based affinity constants $K_L$ (since $K_{L,\text{mass}} = K_{L,\text{molar}} / M_W$, where $M_W$ is the molecular weight), leading to an artificial difference in their mass-based $E_{\mathrm{ads}}$. Therefore, to achieve true thermodynamic comparability across different chemical species, concentrations must be expressed on a molar basis (e.g., mol~L$^{-1}$), and the standard-state concentration must be defined as $C^0 = 1$~mol~L$^{-1}$ (or 1~M). A mass-based standard state (such as $C^0 = 1$~mg~L$^{-1}$) is only acceptable for benchmarking different adsorbents against the same single pollutant, where the adsorbate's molecular weight is constant and relative energy differences are preserved.
...
\begin{comment}
	...
	\begin{table}[H]
		\small
		\caption{Energy-distribution families and heterogeneity width $\sigma_E$ implied by each isotherm model. Parameter $m$ (Sips, Koble--Corrigan) is the heterogeneity exponent; $t$ (T\'{o}th) and $n_H$ (Hill) are the respective shape parameters. $\sigma_E$ is in units of $RT$ ($RT\approx 2.48$~kJ\,mol$^{-1}$ at $298$~K). Full derivations: Section~\ref{SI-sec:si_individual_distributions}.}
...
\begin{comment}
	...
	\begin{itemize}
		...
		\item $\sigma_E$ --- standard deviation of the site-energy distribution (kJ\,mol$^{-1}$), quantifying the degree of surface heterogeneity.
	\end{itemize}
...
\begin{comment}
	...
	\begin{enumerate}
		...
		\item \textbf{Bootstrap validation.} Resample residuals ($\geq 1000$ iterations) to obtain confidence intervals on all fitted parameters. Propagate the bootstrap ensemble through the descriptor extraction formulas (Table~\ref{tab:extraction}) to obtain uncertainties on $E_{\mathrm{ads}}$ and $\sigma_E$.
...
Importantly, the use of information criteria does not imply that one model is “true,” but rather identifies the model that achieves the best trade-off between accuracy and parsimony within the candidate set. In the context of adsorption isotherms, this is essential to avoid overinterpreting parameters obtained from overly flexible models that fit noise rather than underlying adsorption behavior.
	...
	\begin{itemize}
		\item \textbf{Langmuir vs.\ Sips:} Accept Langmuir if the Sips-derived heterogeneity is negligible, $\sigma_E^{\mathrm{(Sips)}} = (\pi/\sqrt{3})\,RT/m \leq \alpha\,RT$ with $\alpha \approx 2$ (equivalently, $m \geq 0.9$; Eq.~\ref{SI-eq:si_criterion_LvsS}).
		...
		\item \textbf{Sips vs.\ Double Langmuir:} Prefer Sips when the site-energy separation is smaller than the distribution width, $\Delta E_{12} < 2\,\sigma_E^{\mathrm{(Sips)}}$ (Eq.~\ref{SI-eq:si_criterion_SvsDL}).
	\end{itemize}
	...
	Once the universal descriptors $\{Q_{\max},\, \ln K_{\mathrm{aff}}^{\ast},\, \sigma_E\}$ (Eq.~\eqref{eq:Eads}) have been extracted for a collection of adsorbent--adsorbate systems, materials can be classified in this descriptor space without imposing a parametric model on the group boundaries. The $k$-Nearest Neighbours (KNN) algorithm assigns a query material to the class most common among its $k$ closest neighbours:
	\begin{equation}
		\hat{y}(\mathbf{x}_q)
		= \arg\max_{c}\;
	...
	\begin{itemize}
		...
		\item \textbf{Homogeneity screening:} separating materials with near-homogeneous surfaces ($\sigma_E \approx 0$) from those with broad energy distributions ($\sigma_E \gg RT$).
	\end{itemize}
	...
	\begin{itemize}
		...
		\item \textbf{Model-type separation:} distinguishing systems that are best described by a Langmuir-type isotherm ($\sigma_E \approx 0$) from those requiring a heterogeneous model (Sips, T\'{o}th).
	...
	\begin{enumerate}
		...
		\item \textbf{Universal descriptors:} $Q_{\max}$ (mg.g$^{-1}$), $K_{\mathrm{aff}}^{\ast}$ (dimensionless), $E_{\mathrm{ads}}$ (kJ\,mol$^{-1}$), and $\sigma_E$ (kJ\,mol$^{-1}$).
	...
	\begin{enumerate}
		...
		\item \textbf{Descriptor extraction.} From the selected model, compute $Q_{\max}$, $K_{\mathrm{aff}}^{\ast}$, $E_{\mathrm{ads}}$, and $\sigma_E$ (Table~\ref{tab:extraction}).
	...
	\begin{itemize}
		...
		\item \textbf{Report confidence intervals.} Bootstrap or Monte Carlo uncertainties on $Q_{\max}$, $E_{\mathrm{ads}}$, and $\sigma_E$ should accompany every table entry.
		\item \textbf{Use $\sigma_E$ to assess model necessity.} If the best 3-parameter model yields only weak heterogeneity, e.g.\ $\sigma_E \leq 2RT$ or, for Sips, $m$ remains close to 1, the Langmuir model is likely sufficient and the extra parameter is not justified (Eq.~\ref{SI-eq:si_criterion_LvsS}).
