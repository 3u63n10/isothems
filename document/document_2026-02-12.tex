...
%	\subsection{Distribution broadening in function of temperature}
	...
	The preceding sections show that every isotherm model that admits a
	finite affinity constant can be brought into a common energetic
	...
	\begin{equation}
		E_c = RT\ln(K_{\mathrm{eq}}\,C^{\ast})
	\end{equation}
	locates the centre (or mode) of the distribution, where
	$K_{\mathrm{eq}}$ is an effective equilibrium constant with units
	$[C]^{-1}$. For models whose native affinity constant already
	has units $[C]^{-1}$ (Langmuir, T\'oth, Jovanovic), $K_{\mathrm{eq}}$
	is the constant itself. For models with fractional exponents
	(Sips, Redlich--Peterson), a unit-corrected effective constant
	$K_S^{1/n}$ or $a_{RP}^{1/\beta}$ is required.

	To report energies on a common scale, a standard concentration
	$C^\circ$ is introduced and the \emph{affinity energy} is defined
	as
	\begin{equation}
		\frac{E_0}{RT}
		= \ln(K_{\mathrm{eq}}\,C^\circ).
	\end{equation}

	Table~\ref{tab:energy_summary} collects the results for all models
	discussed in this document.

	\begin{table}[htbp]
		\centering
		\small
		\caption{Concentration normalisation of adsorption energy:
			effective equilibrium constant $K_{\mathrm{eq}}$, energy
			distribution type, and standard deviation $\sigma_E$ for
			each isotherm model. The affinity energy is
			$E_0/RT = \ln(K_{\mathrm{eq}}\,C^\circ)$ in all cases.}
		\label{tab:energy_summary}
		\renewcommand{\arraystretch}{1.35}
		\begin{tabular}{lcccc}
			\hline
			\textbf{Model}
			& $\boldsymbol{K_{\mathrm{eq}}}$
	...
	The key insight from this table is that models with the same
	dimensional structure for $K_{\mathrm{eq}}$ (Langmuir, T\'oth,
	Jovanovic) yield energies that are directly comparable via
	$E_0 = RT\ln(K_{\mathrm{eq}}\,C^\circ)$ without any exponent
	correction. Models with fractional exponents (Sips, Hill,
	Redlich--Peterson) require the normalisation
	$K_{\mathrm{eq}} = K^{1/n}$ to convert units to $[C]^{-1}$
	before the energy can be computed. The Freundlich model is
	degenerate in that both $Q_{\max}$ and $K_H$ diverge, so the
	affinity energy is concentration-dependent and no single
	$E_0$ can be defined.
