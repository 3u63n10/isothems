...
\begin{figure}[p]
	...
	\begin{tikzpicture}[
		...
		decision/.style={draw, diamond, aspect=2.2, inner sep=1pt,
			minimum width=2.8cm, align=center, thick, fill=yellow!15},
		result/.style={draw, rounded corners, minimum width=2.0cm,
			minimum height=0.6cm, align=center, thick, fill=green!15},
		process/.style={draw, rounded corners, minimum width=2.5cm,
			minimum height=0.6cm, align=center, thick, fill=blue!10},
		phase/.style={draw=none, fill=gray!15, rounded corners,
			inner sep=6pt},
		...
		% --- Q1: saturation ---
		\node[decision, below=0.6cm of fit] (q1)
		{Data shows\\saturation plateau?};

		% NO saturation branch (right)
		\node[decision, right=2.8cm of q1] (q1b)
		{Intermediate\\$C_e$ range only?};
		\node[result, right=2.2cm of q1b] (r_temkin)
		{\textbf{Temkin}\\$q_e = \frac{RT}{b_T}\ln(a_T C_e)$\\
			uniform $f(E)$};
		\node[result, below=0.5cm of q1b] (r_freund)
		{\textbf{Freundlich}\\$q_e = K_F C_e^{1/n}$\\
			power-law $f(E)$, no $Q_{\max}$};

		% YES saturation branch (down)
		\node[decision, below=0.8cm of q1] (q2)
		{Microporous\\adsorbent?};

		% DR/DA branch
		\node[decision, right=2.8cm of q2] (q2b)
		{F-test:\\D-A better\\than D-R?};
		\node[result, right=2.0cm of q2b] (r_DA)
		{\textbf{D-A}\\Weibull $f(\varepsilon)$};
		\node[result, above=0.4cm of r_DA] (r_DR)
		{\textbf{D-R}\\Rayleigh $f(\varepsilon)$};

		% Continue down
		\node[decision, below=0.8cm of q2] (q3)
		{Isotherm\\sigmoidal\\(S-shaped)?};

		% Hill branch
		\node[result, right=2.8cm of q3] (r_hill)
		{\textbf{Hill} ($n_H > 1$)\\cooperative binding\\
			narrow logistic $f(E)$};

		% Continue: AIC ranking of saturating models
		\node[process, below=0.8cm of q3] (aic)
		{\textbf{Rank by AIC$_c$:}\\
			Langmuir, Sips, T\'{o}th,\\
			Jovanovic, UNILAN,\\
			Double Langmuir, R--P};

		% ============ PHASE 2: Pairwise tests ============
		% --- Langmuir vs heterogeneous ---
		\node[decision, below=0.8cm of aic] (d1)
		{Best 3-param model\\$\Delta\mathrm{AIC}$ vs.\ Langmuir $< 2$?};
		\node[result, right=3.5cm of d1] (r_lang)
		{\textbf{Langmuir}\\$\delta(E - E_c)$,\; $\sigma_E = 0$};

		% F-test
		\node[decision, below=0.7cm of d1] (d2)
		{F-test for\\nested pair\\significant?};
		\node[result, right=3.5cm of d2] (r_lang2)
		{Langmuir adequate\\(simpler model)};

		% --- Which heterogeneous model? ---
		\node[decision, below=0.7cm of d2] (d3)
		{Which model\\has lowest AIC$_c$?};

		% Sips branch
		\node[decision, below left=0.9cm and 2.5cm of d3] (d_sips)
		{$\sigma_E^{\mathrm{(Sips)}}$\\$= \frac{\pi n}{\sqrt{3}}RT$\\
			$\leq \alpha\,RT$?};
		\node[result, below=0.5cm of d_sips] (r_sips)
		{\textbf{Sips}\\logistic $f(E)$\\
			$\sigma_E = \frac{\pi n}{\sqrt{3}}RT$};
		\node[result, left=1.5cm of d_sips] (r_sips_neg)
		{Langmuir +\\``negligible\\heterogeneity''};

		% Toth branch
		\node[decision, below=0.9cm of d3] (d_toth)
		{T\'{o}th: $t$ close\\to 1?};
		\node[result, below=0.5cm of d_toth] (r_toth)
		{\textbf{T\'{o}th}\\gen.\ logistic $f(E)$\\
			asymm.\ (low-$E$ tail)};

		% Right branch: Jov / DL / UNILAN / RP
		\node[decision, below right=0.9cm and 2.5cm of d3] (d_right)
		{Model?};

		\node[result, right=1.0cm of d_right, yshift=1.2cm] (r_jov)
		{\textbf{Jovanovic}\\Gumbel $f(E)$\\
			$\sigma_E = \frac{\pi}{\sqrt{6}}RT$};
		\node[result, right=1.0cm of d_right, yshift=0.0cm] (r_unilan)
		{\textbf{UNILAN}\\uniform $f(E)$\\(exact kernel)};
		\node[result, right=1.0cm of d_right, yshift=-1.2cm] (r_RP)
		{\textbf{Redlich--Peterson}\\non-standard $f(E)$\\
			($\beta \to 1$: Langmuir)};

		% Double Langmuir branch
		\node[decision, below=0.5cm of d_right] (d_DL)
		{$\Delta E_{12}$\\$> \beta\,RT$?};
		\node[result, below=0.5cm of d_DL] (r_DL)
		{\textbf{Double Langmuir}\\two $\delta$-functions\\
			$\Delta E_{12} = RT|\ln(K_1/K_2)|$};
		\node[result, left=1.5cm of d_DL] (r_DL_no)
		{Sites thermally\\indistinguishable\\$\to$ Langmuir};
		...
		\draw[arr] (q1b) -- node[nolbl, left] {no} (r_freund);
		\draw[arr] (q1) -- node[yeslbl, left] {yes} (q2);
		\draw[arr] (q2) -- node[yeslbl, above] {yes} (q2b);
		\draw[arr] (q2b) -- node[yeslbl, above] {yes} (r_DA);
		\draw[arr] (q2b) -- node[nolbl, above left] {no} (r_DR);
		...
		% Branching from d3
		\draw[arr] (d3) -| node[yeslbl, above left, pos=0.25]
		{Sips} (d_sips);
		\draw[arr] (d3) -- node[yeslbl, left, pos=0.3]
		{T\'{o}th} (d_toth);
		\draw[arr] (d3) -| node[yeslbl, above right, pos=0.25]
		{Jov/UNILAN/RP/DL} (d_right);
		...
		% Toth sub-tree
		\draw[arr] (d_toth) -- node[nolbl, left] {no ($t \ll 1$)} (r_toth);
		\draw[arr] (d_toth.east) -- ++(1.2,0) |- (r_lang.south west)
		node[yeslbl, pos=0.15, above] {yes ($t\!\approx\!1$)};

		% Right sub-tree
		\draw[arr] (d_right) -- node[yeslbl, above, pos=0.3] {Jov} (r_jov);
		\draw[arr] (d_right) -- node[yeslbl, above, pos=0.3] {UNI} (r_unilan);
		\draw[arr] (d_right) -- node[yeslbl, below, pos=0.3] {R--P} (r_RP);
		\draw[arr] (d_right) -- node[nolbl, left] {DL} (d_DL);
		\draw[arr] (d_DL) -- node[nolbl, left] {yes} (r_DL);
		\draw[arr] (d_DL) -- node[yeslbl, above] {no} (r_DL_no);

		% ============ Phase labels ============
		\begin{scope}[on background layer]
			\node[phase, fit=(fit)(q1)(q3)(r_hill)(r_temkin)(r_DR)(r_DA),
	...
	\caption{Complete decision tree for isotherm model selection.
		...
		\textbf{Phase~2} (middle): AIC$_c$ ranking of all saturating
		models.
		\textbf{Phase~3} (bottom): pairwise nested-model tests (F-test)
		combined with physical criteria ($\sigma_E$, $\Delta E_{12}$,
		$t$, $\beta$).  Terminal boxes (green) show the accepted model
		and its energy-distribution family.}
...
% \input{energy_distributions_params.tex}
...
% === Auto-generated table from MATLAB ===
% \input{energy_distributions_sigma_values.tex}
